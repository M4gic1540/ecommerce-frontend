...
const apiUrl = 'http://localhost:8000/api/productos/productos/?categoria=PC';
...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        const productospc = response.data.results || response.data; // Ajusta según la estructura real de la respuesta
        res.render('pc', { productospc });
    } catch (error) {
        console.error('Error fetching pc products:', error);
        res.status(500).send('Error al obtener los productos de pc');
    }
});
